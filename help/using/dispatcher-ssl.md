...
seo-title: Utilizar SSL con Dispatcher
description: Aprenda a configurar Dispatcher para que se comunique con AEM mediante conexiones SSL.
seo-description: Aprenda a configurar Dispatcher para que se comunique con AEM mediante conexiones SSL.
...
* Versiones de AEM anteriores: consulte [esta página](https://helpx.adobe.com/es/experience-manager/aem-previous-versions.html).
...
1. [Cree un repositorio de claves que contenga certificados de procesamiento](dispatcher-ssl.md#main-pars-title-6) y configure el servicio HTTP del procesamiento para utilizarlo.
...
1. Cambie el nombre de newcert.pem a rendercert.pem y el de newkey.pem a renderkey.pem.
...
1. Cambie el nombre de newcert.pem a dispcert.pem y el de newkey.pem a dispkey.pem.
...
#### Convertir el certificado de procesamiento al formato JKS {#converting-the-render-certificate-to-jks-format}
...
1. Introduzca el siguiente comando para convertir el certificado de procesamiento, que es un archivo PEM, a un archivo PKCS#12. Incluya también el certificado de la CA que firmó el certificado de procesamiento:
...
1. El Java Keystore se crea con un alias predeterminado. Puede cambiar el alias si lo desea:
