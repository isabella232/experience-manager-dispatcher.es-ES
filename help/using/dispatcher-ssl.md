...
Si actúa como CA, utilice [OpenSSL](https://www.openssl.org/) para crear la entidad emisora de certificados que firma los certificados de servidor y cliente. (Debe tener instaladas las bibliotecas OpenSSL). Si utiliza una CA de terceros, no realice este procedimiento.
...
1. Utilice un editor de texto o el `cat` comando para combinar la clave privada no cifrada y el certificado en un único archivo similar al siguiente ejemplo:
