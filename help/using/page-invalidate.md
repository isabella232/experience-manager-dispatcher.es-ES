...
La cuenta `admin` de usuario predeterminada se utiliza para autenticar los agentes de replicación instalados de forma predeterminada. Debe crear una cuenta de usuario dedicada para utilizarla con agentes de replicación.
...
Para obtener más información sobre cómo habilitar el acceso a las direcciones URL personales, consulte [Activación del acceso a las direcciones URL](dispatcher-configuration.md#enabling-access-to-vanity-urls-vanity-urls)de vanidad.
...
Todos los demás archivos de la caché del despachante (o hasta un nivel determinado, según la `/statfileslevel` configuración) se invalidan al tocar el `.stat` archivo. La fecha de la última modificación de este archivo se compara con la fecha de la última modificación de un documento en caché y el documento se recupera si el `.stat` archivo es más reciente. Consulte [Invalidación de archivos por nivel](dispatcher-configuration.md#main-pars_title_26) de carpeta para obtener más información.
...
**Nota:** La eliminación y el registro de archivos deben realizarse únicamente en la instancia de publicación. Cuando se realiza desde la instancia de autor, las condiciones de carrera se dan cuando se intenta recuperar los recursos antes de que se publiquen.
