...
* Si el archivo de configuración es grande, puede dividirlo en varios archivos más pequeños (que sean más fáciles de administrar) e incluirlos cada uno.
...
Por ejemplo, si el archivo dispatcher.any está en el mismo directorio que el directorio de caché, el siguiente valor para la variable [docroot](#specifying-the-cache-directory) se puede utilizar:
...
* Otros comportamientos, como qué archivos almacenar en caché y dónde almacenar en caché.
...
>[!NOTE]
>
>Si utiliza más de una granja de servidores de procesamiento, la lista se evaluará de abajo hacia arriba. Este flujo es relevante a la hora de definir [Hosts virtuales](#identifying-virtual-hosts-virtualhosts) para sus sitios web.
...
>[!CAUTION]
>
>El parámetro `/homepage` (solo IIS) ya no funciona. En su lugar, debe utilizar el [Módulo de reescritura de URL de IIS](https://learn.microsoft.com/en-us/iis/extensions/url-rewrite-module/using-the-url-rewrite-module).
>
...
Por ejemplo, una instancia de Dispatcher que administra las solicitudes de activación de página para instancias de publicación requiere el encabezado `PATH` en la sección `/clientheaders`. La variable `PATH` permite la comunicación entre el agente de replicación y Dispatcher.
...
La siguiente configuración de ejemplo gestiona las solicitudes de `.com` y `.ch` dominios de myCompany y todos los dominios de mySubDivision:
...
* Si no `virtualhosts` los valores tienen `scheme` y `uri` partes que coinciden con el `scheme` y `uri` del host virtual de la solicitud que se encuentra por primera vez que coincide con la variable `host` de la solicitud.
...
>[!CAUTION]
>
> Al configurar el subparámetro de directorio, **no** apunte a la carpeta raíz (`/directory "/"`) ya que puede causar problemas graves. Especifique siempre la ruta a la carpeta que almacena la información de la sesión. Por ejemplo:
...
El nombre del encabezado HTTP o la cookie que almacena la información de autorización. Si almacena la información en el encabezado http, utilice `HTTP:<header-name>`. Para almacenar la información en una cookie, utilice `Cookie:<header-name>`. Si no especifica ningún valor, `HTTP:authorization` se utiliza.
...
Si la variable `/secure` tiene un valor de `"1"`, Dispatcher utiliza HTTPS para comunicarse con la instancia de AEM. Para obtener más información, consulte también [Configuración de Dispatcher para usar SSL](dispatcher-ssl.md#configuring-dispatcher-to-use-ssl).

**/always-resolve**

Con la versión de Dispatcher **4.1.6**, puede configurar la propiedad `/always-resolve` de la siguiente manera:

* Cuando se configura como `"1"`, resuelve el nombre de host en cada solicitud (Dispatcher nunca almacena en caché ninguna dirección IP). Puede afectar ligeramente al rendimiento debido a la llamada adicional necesaria para obtener la información del host de cada solicitud.
* Si la propiedad no está establecida, la dirección IP se almacena en caché de forma predeterminada.
...
La variable `/filter` consiste en una serie de reglas que deniegan o permiten el acceso al contenido según los patrones de la parte de la línea de solicitud de la solicitud HTTP. Utilice una estrategia de lista de permitidos para su `/filter` sección:
...
* **Elementos avanzados de la línea de solicitud:** a partir de Dispatcher 4.2.0, hay cuatro elementos de filtro nuevos disponibles. Estos nuevos elementos son `/path`, `/selectors`, `/extension`y `/suffix` respectivamente. Incluya uno o más de ellos para controlar aún más los patrones de URL.
...
Los patrones deben tener en cuenta los caracteres de espacio de la línea de solicitud y de la variable `<CRLF>` caracteres.
...
Si los filtros no se activan del modo esperado, habilite [Seguimiento del registro](#trace-logging) en Dispatcher para que pueda ver qué filtro está interceptando la solicitud.

#### Filtro de ejemplo: Denegar todo {#example-filter-deny-all}

Este filtro de ejemplo hace que Dispatcher rechace las solicitudes de todos los archivos. Deniegue el acceso a todos los archivos y luego permita el acceso a áreas específicas.
...
Si debe acceder a páginas únicas dentro del área restringida, puede permitir el acceso a ellas. Por ejemplo, para permitir el acceso a la pestaña Archivo dentro de la consola de flujo de trabajo, agregue la siguiente sección:
...
>[!NOTE]
>
>Cuando se aplican varios patrones de filtros a una solicitud, el último patrón de filtro aplicado es efectivo.
...
A continuación, se muestra un ejemplo de regla que bloquea la captura de contenido desde el `/content` ruta y su subárbol, utilizando filtros para ruta, selectores y extensiones:
...
Después de crear los filtros, [probar el acceso a la página](#testing-dispatcher-security) para garantizar que la instancia de AEM sea segura.
...
Deniegue el acceso a todo y, a continuación, permita el acceso a elementos específicos (limitados):
...
* Deshabilitar el acceso externo a `/admin` si utiliza la versión 5.4 de CQ o una versión anterior.
...
>[!CAUTION]
>
>Si [uso de informes en un entorno de publicación](https://experienceleague.adobe.com/docs/experience-manager-65/administering/operations/reporting.html?lang=es#using-reports-in-a-publish-environment), debe configurar Dispatcher para que deniegue el acceso a `/etc/reports` para visitantes externos.
...
>[!NOTE]
>
>Si una regla contiene una `/query`, solo coincide con las solicitudes que contienen una cadena de consulta y que coinciden con el patrón de consulta proporcionado.
>
...
Debería ver la renderización de página normal para `/content/add_valid_page.html?debug=layout`.
...
Las solicitudes de distribución solo están destinadas a Dispatcher, por lo que de forma predeterminada no se envían al procesador (por ejemplo, una instancia de AEM).
...
Si no tiene páginas dinámicas (más allá de las páginas ya excluidas por las reglas anteriores), puede configurar Dispatcher para que almacene todo en caché. La sección de reglas tiene el siguiente aspecto:
...
>[!NOTE]
>
>No almacene en caché los grupos de usuarios cerrados, ya que los derechos de usuario no se comprueban para las páginas en caché.
...
* Cuando se invalida un archivo de un nivel determinado, **all** `.stat` archivos de docroot **a** el nivel del archivo no válido o configurado `statsfilevel` (lo que sea más pequeño) se tocan.

   * Por ejemplo: si configura la variable `statfileslevel` propiedad a 6 y un archivo se invalida en el nivel 5, entonces cada `.stat` del docroot al 5 se tocan. Continuando con este ejemplo, si un archivo se invalida en el nivel 7, entonces cada `stat` del docroot a seis se tocan (ya que `/statfileslevel = "6"`).
...
Cuando un archivo `/content/myWebsite/xx` se invalida, entonces cada `.stat` desde docroot hasta `/content/myWebsite/xx`es tocado. Este escenario solo es así para `/content/myWebsite/xx` y no, por ejemplo `/content/myWebsite/yy` o `/content/anotherWebSite`.

>[!NOTE]
>
>La invalidación se puede evitar enviando un encabezado adicional `CQ-Action-Scope:ResourceOnly`. Este método se puede utilizar para vaciar recursos concretos sin invalidar otras partes de la caché. Consulte [esta página](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html) e [Invalidación manual de la caché de Dispatcher](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/page-invalidate.html?lang=es#configuring) para obtener más información.
...
* El resto de archivos que coinciden con la configuración `/invalidate` no se eliminarán inmediatamente. Estos archivos se eliminarán cuando se produzca la siguiente solicitud. En el ejemplo `/content/wknd.html` no se suprime; se elimina cuando `/content/wknd.html` se solicita.

Si ofrece archivos PDF y ZIP generados automáticamente para su descarga, es posible que también tenga que invalidar automáticamente estos archivos. Un ejemplo de configuración tiene el siguiente aspecto:
...
Este método se puede utilizar para cubrir varios casos de uso diferentes. Por ejemplo, invalidar otras cachés específicas de la aplicación o manejar casos en los que la URL externalizada de una página, y su lugar en el docroot, no coinciden con la ruta de contenido.
...
#### Secuencia de comandos de controlador de invalidación de muestra {#sample-invalidation-handler-script}
...
>
>Si no se hace, cualquier cliente puede emitir una llamada para borrar la caché. Si se realiza repetidamente, puede afectar gravemente el rendimiento del sitio.
...
>[!NOTE]
>
>Al configurar la propiedad glob, debe coincidir con el nombre del parámetro de consulta. Por ejemplo, si desea ignorar el parámetro &quot;p1&quot; de la siguiente dirección URL `http://example.com/path/test.html?p1=test&p2=v2`, la propiedad glob debe ser:
> `/0002 { /glob "p1" /type "allow" }`

El siguiente ejemplo hace que Dispatcher ignore todos los parámetros, excepto el parámetro `nocache`. Como tal, solicite las direcciones URL que incluyen la variable `nocache` Dispatcher nunca almacena en caché el parámetro:
...
>* Agregue lo siguiente [Directiva Apache](https://httpd.apache.org/docs/2.4/mod/core.html#fileetag) en la sección relacionada con Dispatcher:
>
>```xml
>FileETag none
>```
...
>[!NOTE]
>
>Tenga en cuenta esta configuración `/enableTTL` a 1 habilita el almacenamiento en caché TTL solo en el lado del dispatcher. Como tal, la información TTL contenida en el archivo adicional (ver arriba) no se proporciona a ningún otro agente de usuario que solicite un tipo de archivo de este tipo a Dispatcher. Si desea proporcionar encabezados de almacenamiento en caché a sistemas descendentes como una CDN o un explorador, debe configurar la variable `/cache/headers` en consecuencia.
...
Dispatcher admite un máximo de ocho categorías de estadísticas. Si define más de ocho categorías, solo se utilizan las ocho primeras.
...
La puntuación de la categoría de un render se basa en tiempos de respuesta anteriores y en conexiones fallidas y exitosas anteriores que intenta Dispatcher. Por cada intento, se actualiza la puntuación de la categoría del URI solicitado.
...
La variable `/stickyConnectionsFor` define una carpeta que contiene documentos adhesivos. Se accede a esta propiedad mediante la dirección URL. Dispatcher envía todas las solicitudes, desde un solo usuario que se encuentra en esta carpeta, a la misma instancia de renderización. Las conexiones duraderas garantizan que los datos de la sesión estén presentes y sean coherentes en todos los documentos. Este mecanismo utiliza la cookie `renderid`.
...
Cuando las conexiones duraderas están habilitadas, el módulo de Dispatcher establece la variable `renderid` cookie. Esta cookie no tiene la variable `httponly` , que debe añadirse para mejorar la seguridad. Agregue la variable `httponly` estableciendo el `httpOnly` en la variable `/stickyConnections` nodo de a `dispatcher.any` archivo de configuración. El valor de la propiedad (`0` o `1`) define si la cookie `renderid` tiene el atributo `HttpOnly` anexado. El valor predeterminado es `0`, lo que significa que el atributo no se agrega.

Para obtener más información sobre el indicador `httponly`, consulte [esta página](https://www.owasp.org/index.php/HttpOnly).

### secure {#secure}

Cuando las conexiones duraderas están habilitadas, el módulo de Dispatcher establece la variable `renderid` cookie. Esta cookie no tiene la variable `secure` , que debe añadirse para mejorar la seguridad. Agregue la variable `secure` indicador que configura la variable `secure` en la variable `/stickyConnections` nodo de a `dispatcher.any` archivo de configuración. El valor de la propiedad (`0` o `1`) define si la cookie `renderid` tiene el atributo `secure` anexado. El valor predeterminado es `0`, lo que significa que se agrega el atributo **if** la solicitud entrante es segura. Si el valor está establecido en `1`, luego se agrega el indicador seguro independientemente de si la solicitud entrante es segura o no.
...
Para reenviar solicitudes a diferentes renderizaciones cuando falla la solicitud original, habilite el mecanismo de conmutación por error en su granja de Dispatcher. Cuando la conmutación por error esté habilitada, Dispatcher se comportará de la siguiente forma:
...
>[!CAUTION]
>
>Esta opción no es necesaria. Utilícelo solo cuando vea los siguientes mensajes de registro:
>
>`Error while reading response: Interrupted system call`

Se puede interrumpir cualquier llamada al sistema orientada al sistema de archivos `EINTR` si el objeto de la llamada del sistema está en un sistema remoto al que se accede mediante NFS. El tiempo de espera de estas llamadas o su interrupción se deben a cómo se montó el sistema de archivos subyacente en el equipo local.
...
Si utiliza un **Apache** servidor web, puede utilizar la funcionalidad estándar para registros girados, registros canalizados o ambos. Por ejemplo, con registros canalizados:
...
>[!NOTE]
>
>Después de la instalación, el nivel de registro predeterminado es alto (es decir, nivel 3 = Depuración), por lo que Dispatcher registra todos los errores y advertencias. Este nivel es útil en las etapas iniciales.
>
>Sin embargo, ese nivel requiere recursos adicionales. Cuando Dispatcher funciona sin problemas *según sus necesidades*, puede reducir el nivel de registro.
...
Esta capacidad es superior al registro de depuración, que muestra información adicional en los registros. Añade el registro para:
...
1. Si todo funciona correctamente, puede reducir la `loglevel` a `0`.
...
Al añadir el encabezado `X-Dispatcher-Info` a una solicitud, Dispatcher responde si el destinatario se ha almacenado en caché, se ha devuelto desde la caché o no se puede almacenar en caché. El encabezado de respuesta `X-Cache-Info` contiene esta información en un formato legible. Puede utilizar estos encabezados de respuesta para depurar los problemas que implican respuestas almacenadas en caché por Dispatcher.

Esta funcionalidad no está habilitada de forma predeterminada, por lo que para el encabezado de respuesta `X-Cache-Info` para que se incluya, la granja debe contener la siguiente entrada:
...
Además, el `X-Dispatcher-Info` el encabezado no necesita un valor, pero si utiliza `curl` para realizar pruebas, debe proporcionar un valor que se envíe al encabezado, como:

```xml
curl -v -H "X-Dispatcher-Info: true" https://localhost/content/wknd/us/en.html
```

A continuación se muestra una lista que contiene los encabezados de respuesta que `X-Dispatcher-Info` devuelve:
...
* **no almacenable en caché: no era un GET o un HEAD**
...
* **no almacenable en caché: el destino es un directorio**\
   El archivo de destino es un directorio. Esta ubicación podría señalar a algún error conceptual, en el que una dirección URL y algunas direcciones URL secundarias contienen resultados almacenables en caché. Por ejemplo, si una solicitud a `/test.html/a/file.ext` llega primero y contiene salida almacenable en caché, Dispatcher no puede almacenar en caché el resultado de una solicitud posterior a `/test.html`.
